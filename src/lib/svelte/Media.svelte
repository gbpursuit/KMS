<script lang="ts">
    ...
    let { style = $bindable(), filePath = $bindable(), addStyle = '', activeTab = $bindable(), parsedCSV, ...props } = $props()
...
{:else if style === 'csv'}
    {#if activeTab.toLowerCase() === 'evaluation'}
        <EvaluationSelect filePath={filePath} parsedCSV={parsedCSV} />
    {:else if activeTab.toLowerCase() === 'participants'}
        <ParticipantSelect filePath={filePath} parsedCSV={parsedCSV} />
    {/if}
{/if}
