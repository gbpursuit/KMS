...
<div class="min-h-screen mt-28 px-6 py-10 space-y-10">
    ...
    <div class="flex col w-full gap-8">
        <Button style="submit" addStyle="bg-red-800 font-bold" onclick={() => {if(data.user) changeRole(data.user.id, data.user.roleId, 1)}}>
            {button1Text}
        </Button>
        <Button style="submit" addStyle="bg-green-800 font-bold" onclick={() => {if(data.user) changeRole(data.user.id, data.user.roleId, 2)}}>
            {button2Text}
        </Button>
        <Button style="submit" addStyle="bg-yellow-600 font-bold" onclick={() => {if(data.user) changeRole(data.user.id, data.user.roleId, 3)}}>
            {button3Text}
        </Button>
    </div>
</div>
